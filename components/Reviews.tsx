import Stars from "./Stars";
import Reveal from "./Reveal";
import { reviews, rating } from "@/lib/content";

function GoogleG({ className = "h-5 w-5" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 48 48" aria-hidden="true">
      <path fill="#4285F4" d="M45.12 24.5c0-1.56-.14-3.06-.4-4.5H24v8.51h11.84c-.51 2.75-2.06 5.08-4.39 6.64v5.52h7.11c4.16-3.83 6.56-9.47 6.56-16.17z" />
      <path fill="#34A853" d="M24 46c5.94 0 10.92-1.97 14.56-5.33l-7.11-5.52c-1.97 1.32-4.49 2.1-7.45 2.1-5.73 0-10.58-3.87-12.31-9.07H4.34v5.7C7.96 41.07 15.4 46 24 46z" />
      <path fill="#FBBC05" d="M11.69 28.18C11.25 26.86 11 25.45 11 24s.25-2.86.69-4.18v-5.7H4.34A21.99 21.99 0 002 24c0 3.55.85 6.91 2.34 9.88l7.35-5.7z" />
      <path fill="#EA4335" d="M24 10.75c3.23 0 6.13 1.11 8.41 3.29l6.31-6.31C34.91 4.18 29.93 2 24 2 15.4 2 7.96 6.93 4.34 14.12l7.35 5.7c1.73-5.2 6.58-9.07 12.31-9.07z" />
    </svg>
  );
}

export default function Reviews() {
  return (
    <section className="section bg-cream">
      <div className="container-bm">
        <div className="flex flex-col items-center text-center">
          <p className="eyebrow">What our clients say</p>
          <h2 className="mt-3 text-3xl font-semibold sm:text-4xl">Loved by the brands we work with</h2>
          <div className="mt-5 inline-flex items-center gap-3 rounded-full border border-ink/10 bg-white px-5 py-2.5 shadow-sm">
            <GoogleG />
            <Stars />
            <span className="font-semibold text-navy">{rating.value.toFixed(1)}</span>
            <span className="text-sm text-ink/60">· Rated on Google</span>
          </div>
        </div>

        <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {reviews.map((r, i) => (
            <Reveal key={r.name} delay={(i % 3) * 0.05}>
              <figure className="flex h-full flex-col rounded-2xl border border-ink/10 bg-white p-6 shadow-sm">
                <div className="flex items-center justify-between">
                  <Stars size="text-sm" />
                  <GoogleG className="h-4 w-4" />
                </div>
                <blockquote className="mt-3 flex-1 text-sm leading-relaxed text-ink/80">“{r.text}”</blockquote>
                <figcaption className="mt-4 flex items-center gap-3">
                  <span className="flex h-9 w-9 items-center justify-center rounded-full bg-navy text-sm font-semibold text-white">
                    {r.name.charAt(0)}
                  </span>
                  <span className="text-sm font-semibold text-navy">{r.name}</span>
                </figcaption>
              </figure>
            </Reveal>
          ))}
        </div>
      </div>
    </section>
  );
}
